# main.py - Flask application with Telegram bot using JSON storage
import os
import json
import hashlib
import secrets
...
def handle_callback(call):
    if call.data == "register":
        ...
        # Do nothing button (for completed tasks)
...
def handle_task_completion(message, telegram_id, task_id):
    ...
    # Generate ad completion URL - using ngrok URL or your domain
    # For local testing, you might need to use your ngrok URL
...
def confirm_join_tournament(call):
    tournament_id = int(call.data.split("_")[2])
...
@bot.callback_query_handler(func=lambda call: call.data == "user_rankings")
@admin_required
...
def serve_ad():
    ...
    <body>
        ...
        <script>
            function watchAd() {
                ...
                show_{{ zone }}('pop').then(() => {
                    window.location.href = '/ad_complete?task_id={{ task_id }}&user_id={{ user_id }}&reward={{ reward }}';
                }).catch(e => {
    ...
    ''', zone=zone, task_id=task_id, user_id=user_id, reward=reward)
