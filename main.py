...
def show_admin_tasks(message):
    ...
    if not tasks:
        ...
        for task in tasks:
            ...
            text += f"ID: {task['id']}\n📌 {task['title']}\n💰 Reward: {task['reward']} coins\n🔄 Max Views: {task['max_views_per_user']}\n{status} | {one_time}\n\n"
    ...
    for task in tasks:
        markup.add(
            InlineKeyboardButton(f"✏️ Edit Task {task['id']}", callback_data=f"edit_task_{task['id']}"),
...
def show_admin_tournaments(message):
    ...
    if not tournaments:
        ...
        text = "🏆 *Manage Tournaments*\n\n"
        for t in tournaments:
            text += f"ID: {t['id']}\n📌 {t['name']}\n💰 Entry: {t['entry_fee']} coins | Prize: {t['prize_pool']}\n📊 Status: {t['status']}\n\n"
...
def add_coins_find_user(message):
    ...
    if len(found_users) == 1:
        user = found_users[0]
        msg = bot.send_message(message.chat.id, f"Found user: {user['ff_name']}\nEnter amount of coins to add:")
        bot.register_next_step_handler(msg, add_coins_amount, user["id"])
    else:
        markup = InlineKeyboardMarkup()
        for user in found_users:
            markup.add(InlineKeyboardButton(user["ff_name"], callback_data=f"select_user_{user['id']}"))
